...
@Component({
  selector: 'app-sales-api-monthly',
  templateUrl: './sales-api-monthly.component.html',
  styleUrls: ['./sales-api-monthly.component.scss']
})
export class SalesApiMonthlyComponent implements OnInit {
  loading = true;
  ...
  ngOnInit() {
    this.getSyncJobs(Constants.POS_SALES_API_MONTHLY_SYNC);
    this.state = localStorage.getItem('getPosSalesLoading');
    if (this.state == "true") {
      SalesApiMonthlyComponent.getPosSalesLoading = true;
    }
    else{
      SalesApiMonthlyComponent.getPosSalesLoading = false;
    }
  }
  ...
  get staticgetPosSalesLoading() {
    return SalesApiMonthlyComponent.getPosSalesLoading ;
  }

  getPOSSales() {
    this.spinner.show();
    this.syncJobService.getSyncJobData(Constants.POS_SALES_API_MONTHLY_SYNC).toPromise().then((res) => {
      this.posSales = res;
  ...
  getPOSSalesSyncJob() {
    ...
    SalesApiMonthlyComponent.getPosSalesLoading = true;
    this.posSalesService.getPOSSalesAPIDaily("Monthly").toPromise().then((res: any) => {
      this.getSyncJobs(Constants.POS_SALES_API_MONTHLY_SYNC);
  ...
  exportToCSV():void {
  //   this.csvService.exportSalesToCSV(this.selectedJob.id, Constants.POS_SALES_API_MONTHLY_SYNC).subscribe(
  //     res => {
