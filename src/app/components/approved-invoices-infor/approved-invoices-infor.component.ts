...
export class ApprovedInvoicesInforComponent implements OnInit {
  ...
  getApprovedInvoicesDB() {
    ...
    this.syncJobService
      .getSyncJobData('Approved Invoices')
      .toPromise()
      .then((res: any) => {
  ...
  exportToXLSX(): void {
    this.excelService.exportInvoicesToExcel(this.selectedJob.id).subscribe(
      (res) => {
        ...
        const file = new File([blob], 'ApprovedInvoices' + '.xlsx', {
  ...
  exportToCSV(): void {
    this.csvService
      .exportSalesToCSV(this.selectedJob.id, Constants.APPROVED_INVOICES_SYNC)
      .subscribe(
        (res) => {
  ...
  generateSingleFile(): void {
    this.csvService
      .generateSingleFile(Constants.APPROVED_INVOICES_SYNC)
      .subscribe(
        (res) => {
          const blob = new Blob([res.body], { type: 'application/vnd.ms.txt' })
          const file = new File(
            [blob],
            Constants.APPROVED_INVOICES_SYNC + '.ndf',
            { type: 'application/vnd.ms.txt' },
          )
          saveAs(file)
