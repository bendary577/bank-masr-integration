...
@Component({
  selector: 'app-add-canteen-user',
  templateUrl: './add-canteen-user.component.html',
  styleUrls: ['./add-canteen-user.component.scss'],
})
export class AddCanteenUserComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddCanteenUserComponent>,
    ...
    if (this.data != undefined && this.data['user'] != null) {
      ...
      for (var i = 0; i < this.user.accompaniedGuests.length; i++) {
        ...
        accompiendForm = this.formBuilder.group({
          name: [
            this.user.accompaniedGuests[i].name,
            [Validators.maxLength, Validators.required],
          ],
          email: [
            this.user.accompaniedGuests[i].email,
            [
              Validators.pattern(
          ...
          mobile: [
            this.user.accompaniedGuests[i].mobile,
            [
      ...
      let expirationDate = moment(this.user.expiryDate).format(
        'YYYY-MM-DDT00:00',
      )
  ...
  validateFrom() {
    for (let i = 0; i < this.accompiendForms.length; i++) {
      if (this.accompiendForms[i].invalid) {
        this.snackBar.open(
          'Please full fill all fields of the accompanied guest correctly.',
          null,
          {
            duration: 3000,
            horizontalPosition: 'center',
